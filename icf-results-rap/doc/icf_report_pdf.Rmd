---
title: "UK International Climate Finance Results"
...
knitr::opts_chunk$set(echo = TRUE, dpi=300, fig.pos = "H", out.extra = "")
...
tar_load(summary_main, store = "../_targets/")
tar_load(summary_results_last, store = "../_targets/")
tar_load(summary_kpi_progs, store = "../_targets/")
tar_load(summary_disagg_main, store = "../_targets/")
tar_load(summary_disagg_prog_count, store = "../_targets/")
tar_load(summary_top_progs, store = "../_targets/")
tar_load(summary_kpi15, store = "../_targets/")

tar_load(kpi_totals_table, store = "../_targets/")
tar_load(disagg_table, store = "../_targets/")
...
tar_load(prog_list_table, store = "../_targets/")
tar_load(tpb_table, store = "../_targets/")

tar_load(line_plot, store = "../_targets/")
tar_load(kpi15_plot_new, store = "../_targets/")
tar_load(kpi15_plot_old, store = "../_targets/")
tar_load(ta01_plot, store = "../_targets/")

tar_load(reporting_periods, store = "../_targets/")
...
>\textit{Our planet has just endured a season of simmering — the hottest summer on record. Climate breakdown has begun. Scientists have long warned what our fossil fuel addiction will unleash. Our climate is imploding faster than we can cope with extreme weather events hitting every corner of the planet. Surging temperatures demand a surge in action. Leaders must turn up the heat now for climate solutions. We can still avoid the worst of climate chaos — and we don’t have a moment to lose.}

\begin{flushright}
- António Guterres \footnote{\href{https://www.un.org/sg/en/content/sg/statement/2023-09-06/secretary-generals-message-the-hottest-summer-record}{UN Secretary-General Statement António Guterres}}
\end{flushright}

\hfill\break


The UN has warned that ‘the world is on the brink of a climate catastrophe, and the window to avert it is closing rapidly’\footnote{UN (2022) Sustainable Development Goals Report}. In 2022, the global mean temperature was approximately 1.15°C above the pre-industrial level, making it one of the warmest years on record\footnote{WMO State of the Global Climate 2022}. To limit warming to 1.5°C, as set out in the Paris Agreement, requires a 43% decrease in global greenhouse gas emissions by 2030, and net zero by 2050.  

The sixth assessment from the Intergovernmental Panel on Climate Change (IPCC) sets out how human-induced climate change is already having devastating impacts globally\footnote{IPCC (2022) 6th Assessment Report WGII: Summary for Policymakers}, and yet greenhouse gas emissions have continued to rise. In 2023, the world experienced the hottest summer on record. The wildfires in Europe were devastating with the Greece wildfire being the largest recorded in the EU, and China and America have experienced powerful hurricanes. The impacts of climate change extend across the Sustainable Development Goals (SDGs), with increased extreme weather events already driving mass mortalities in plant and animal species.  

The IPCC report emphasises the interdependence of climate, ecosystems and biodiversity, and human society, and the need for an integrated approach across these areas to solve the challenges we face. The mutually reinforcing crises of rapid climate change and unprecedented biodiversity loss are a key threat to global security and prosperity. Every year the window for action gets smaller, while the risks we face increase and become more complex. Climate change and biodiversity loss are closely linked: we cannot reach net zero or limit global temperature rises to 1.5 degrees without protecting and restoring ecosystems. Therefore, improvements in agriculture, protecting and restoring nature and using land and the ocean sustainably are central to efforts to tackle and adapt to climate change, build resilience against wider shocks and to protect the global economy. By 2030, climate change and environmental degradation could push a further 100 million into poverty \footnote{World Bank (2016) Managing the impacts of climate change on poverty.}. Millions of people are increasingly exposed to food and water insecurity, but it is the poorest and most vulnerable who are hit first, hardest and who are least able to cope\footnote{Dasgupta et. al. (2021) The Economics of Biodiversity}.  

While the IPCC assessment stressed the severity of the situation, it also outlines how it is still possible to achieve the necessary emissions reductions to keep global warming to 1.5°C\footnote{IPCC (2022) 6th Assessment Report WGIII: Summary for Policymakers}. Every 0.1°C of warming avoided will improve our chances of limiting climate catastrophe. Major transitions will be required in the energy sector to reduce our dependence on fossil fuels. Improvements in urban design and transport will be required to reduce energy consumption. More efficient use and recycling of resources in industry will be important to reduce greenhouse gas emissions. Substantial progress was made at the United Nations Climate Change Conference COP26, hosted by the UK in 2021, which included: 

- Securing net zero commitments for over 90% of the world’s economy - up from 30% two years ago, when the UK began preparations for COP26.
...
At the United Nations Climate Change Conference COP 27 hosted in Egypt in 2022 more progress was made, including:
- Agreement on a Loss and Damage Fund for vulnerable countries devastated by extreme weather events.
- Continuation of the 1.5C limit and agreement to strengthen these commitments on cutting greenhouse gases each year.
- World Bank reform to help provide the funding to help vulnerable countries adapt to the impacts of climate change and reduce their greenhouse gas emissions.
- Re-affirmed commitment to increased support to developing countries, including at least doubling adaptation finance to $40 billion per year by 2025.

The combination of net zero targets, enhanced 2030 emissions reduction commitments, protecting and restoring nature and global collaboration in key sectors can put the world on track to limit global warming. However, increased action is needed to keep us to the lower 1.5°C limit set out in the Paris Agreement\footnote{UN (2015) United Nations Framework Convention on Climate Change: Paris Agreement}.
...
Previous publications and the methodologies used to guide results monitoring and collection can be found on the [ICF Results webpages](). Details of the programmes referenced throughout this publication can be found on [Development Tracker](https://devtracker.fcdo.gov.uk/)
...
kpi_totals_table %>% 
  ...
  kable(
    caption = "ICF results achieved from April 2011 to March 2023",
    booktabs=T
  ) %>% 
...
```{r kpi1 chart, echo=FALSE, include=TRUE, fig.cap="Cumulative annual results achieved for KPI 1 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[1]]
```
...
```{r kpi2.1 chart, echo=FALSE, include=TRUE, fig.cap="Cumulative annual results achieved for KPI 2.1 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[2]]
```
...
```{r kpi2.2 chart, echo=FALSE, include=TRUE, fig.cap="Cumulative annual results achieved for KPI 2.2 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[3]]
```
...
```{r kpi4 chart, echo=FALSE, include=TRUE, fig.cap="Cumulative annual results achieved for KPI 4 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[4]]
```
...
```{r kpi6 chart, echo=FALSE, include=TRUE, fig.cap="Cumulative annual results achieved for KPI 6 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[5]]
```
...
```{r kpi7 chart, echo=FALSE, include=TRUE, fig.cap="Cumulative annual results achieved for KPI 7 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[6]]
```
...
```{r kpi8 chart, echo=FALSE, include=TRUE, fig.cap="Cumulative annual results achieved for KPI 8 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[7]]
```
...
```{r kpi10 chart, echo=FALSE, include=TRUE, fig.cap="Cumulative annual results achieved for KPI 10 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[8]]
```
...
```{r kpi11 chart, echo=FALSE, include=TRUE, fig.cap="Cumulative annual results achieved for KPI 11 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[9]]
```
...
```{r kpi12 chart, echo=FALSE, include=TRUE, fig.cap="Cumulative annual results achieved for KPI 12 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[10]]
```
...
```{r kpi17 chart, echo=FALSE, include=TRUE, fig.cap="Cumulative annual results achieved for KPI 17 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[11]]
```
...
```{r ta01_plot, echo=FALSE, fig.cap="Countries support with technical assistance reported by ICF TA KPI 1. Darker shades are countries that have been supported.", fig.align='center'}
ta01_plot
```
...
```{r kpi15_plot_old, echo=FALSE, fig.cap="ICF KPI 15 results up to 2021/2022 using the previous scoring approach.", fig.align='center', fig.width=5.5, fig.height=4}
kpi15_plot_old
```
...
[^2] [UN Secretary-General Statement António Guterres](https://www.un.org/sg/en/content/sg/statement/2023-09-06/secretary-generals-message-the-hottest-summer-record)
[^3] UN (2022) Sustainable Development Goals Report
[^4] WMO State of the Global Climate 2022
[^5] IPCC (2022) 6th Assessment Report WGII: Summary for Policymakers
[^6] World Bank (2016) Managing the impacts of climate change on poverty.
[^7] Dasgupta et. al. (2021) The Economics of Biodiversity
[^8] IPCC (2022) 6th Assessment Report WGIII: Summary for Policymakers
[^9] UN (2022) COP26 Glasgow Climate Pact
[^10] UN (2015) United Nations Framework Convention on Climate Change: Paris Agreement
[^11] The non-continuous numbering of ICF KPIs reflects the discontinuation of indicators found not to be well suited to ICF portfolio monitoring in previous reviews.
[^12] For this KPI, social institutions are defined as: schools, universities, hospitals, health centres, government institutions, state owned infrastructure and civil society organisations
[^13] Attributed means that our work directly and solely caused the results; contributed means our work was part of a number of factors which led to a result.   
[^14] The Clean Development Mechanism (CDM) is a way to finance emissions mitigation projects by selling certified emission reductions, or CERs. For further information, see https://cdm.unfccc.int/
