...
header-includes:
  - \usepackage{fontspec}
  - \setmainfont[SlantedFont=*,SlantedFeatures={FakeSlant=0.2}]{GDSTransportWebsite}
  - \usepackage{booktabs}
  - \usepackage{array}
  - \usepackage{endnotes}
  - \usepackage{epigraph}
  ...
  - \usepackage{xcolor}
  - \definecolor{afblue}{HTML}{12436D}
...
\ThisULCornerWallPaper{1}{../images/cover4.jpg}

\vspace*{13cm}


\noindent{\Huge\textbf{\textcolor{afblue}{UK International Climate Finance}}}\hfill\break
\hfill\break
\noindent{\Huge Results 2023}\hfill\break

\vspace*{5cm}


\begin{minipage}{6in}
  \raisebox{-0.5\height}{\includegraphics[width=0.5\textwidth]{../images/uk_gov.png}}
\end{minipage}
...
\begin{acronym}
\acro{COP}{Conference of the Parties (to the United Nations climate convention)}
\acro{Defra}{Department for Environment, Food and Rural Affairs}
\acro{DESNZ}{Department for Energy Security and Net Zero}
\acro{DSIT}{Department for Science, Innovation and Technology}
\acro{FCDO}{Foreign, Commonwealth and Development Office}
\acro{IPCC}{Intergovernmental Panel on Climate Change}
\acro{KPI}{Key Performance Indicator}
\acro{ODA}{Official Development Assistance}
\acro{REDD}{Reducing emissions from deforestation and forest degradation in developing countries}
\acro{SDG}{Sustainable Development Goal}
\acro{TA}{Technical Assistance}
\acro{UN}{United Nations}
\acro{WASH}{Water, Sanitation and Hygiene}
...
knitr::opts_chunk$set(echo = TRUE, dpi=300) # add following to suppress float: fig.pos = "H", out.extra = "")
...
disagg_table <- disagg_table %>% 
  mutate(`Number of Programmes` = str_replace_all(`Number of Programmes`, "<5", "$<$5")) %>% 
mutate(`Total Achieved` = str_replace_all(`Total Achieved`, "<20", "$<$20")) 
...
The sixth assessment from the Intergovernmental Panel on Climate Change (IPCC) emphasises the interdependence of climate, ecosystems and biodiversity, and human society, and the need for an integrated approach across these areas to solve the challenges we face. The mutually reinforcing crises of rapid climate change and unprecedented biodiversity loss are a key threat to global security and prosperity. We cannot reach net zero or limit global temperature rises to 1.5 degrees without protecting and restoring ecosystems. Therefore, improvements in agriculture, protecting and restoring nature and using land and the ocean sustainably are central to efforts to tackle and adapt to climate change, build resilience against wider shocks and to protect the global economy. By 2030, climate change and environmental degradation could push a further 100 million into poverty\footnote{World Bank (2016) Managing the impacts of climate change on poverty.}. Millions of people are increasingly exposed to food and water insecurity, but it is the poorest and most vulnerable who are hit first, hardest and who are least able to cope\footnote{Dasgupta et. al. (2021) The Economics of Biodiversity}. The UK’s International Development Strategy in 2022 and the Integrated Review Refresh in 2023 set out how tackling, and adapting, to climate change are, and will be, part of the UK’s development offer.

While the IPCC assessment stressed the severity of the situation, it also outlined how it is still possible to achieve the necessary emissions reductions to keep global warming to 1.5°C\footnote{IPCC (2022) 6th Assessment Report WGIII: Summary for Policymakers}. Substantial progress was made at the United Nations Climate Change Conference COP26, hosted by the UK in 2021, which included:
...
- The Glasgow Climate Pact was agreed, which set out ambitious commitments to increase action on climate change\footnote{UN (2022) COP26 Glasgow Climate Pact}, including phasing down coal and ending inefficient fossil fuel subsidies for the first time.
...
ICF Results are reported for 11 KPIs and 4 TA KPIs\footnote{The non-continuous numbering of ICF KPIs reflects the discontinuation of indicators found not to be well suited to ICF portfolio monitoring in previous reviews.}. `r summary_main %>% filter(achieved_total>0) %>% distinct(programme_id) %>% nrow() %>% as.english() %>% str_to_sentence()` programmes have reported ICF results since 2011. Many of these programmes will continue to deliver further benefits after they have ended, so as well as cumulative results to date, we also report on total expected programme benefits (see Annex).

The total cumulative results achieved for each KPI between April 2011 and March `r year(Sys.Date())` are shown in Table \ref{tab:tab1}. Where this year’s KPI results are compared to those from the previous year, the latest available data has been used, including historic revisions, so these differences may not match previously published data. Throughout the publication, all figures greater than 10,000 have been rounded down to the nearest whole thousand.  
...
```{r kpi_totals, echo = FALSE, fig.alt="Cumulative ICF Results from 2011 to the present year. ICF has supported over one hundred and one million people to adapt to the effects of climate change."}

kpi_totals_table %>% 
  mutate(`KPI Title` = str_remove_all(`KPI Title` , " as a result of ICF")) %>% 
  mutate(`KPI Number` = str_replace_all(`KPI Number`, "I 0", "I ")) %>% 
  kable(
    caption = "\\label{tab:tab1}ICF results achieved from April 2011 to March 2023",
    booktabs=T
  ) %>% 
...
```{r kpi1 chart, echo=FALSE, include=TRUE, fig.cap="\\label{fig:fig1}Cumulative annual results achieved for KPI 1 as a result of UK ICF, from April 2011 to March 2023", fig.alt="Cumulative annual results achieved for KPI 1 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[1]]
```



```{r, echo=FALSE, fig.align='center', fig.alt="Data are disaggregated by Age, Disability, Geography and Sex"}
disagg_tab_01 <- disagg_table %>% filter(kpi_id=="ICF KPI 01") %>% select(-kpi_id)

kable(disagg_tab_01[,-1], 
      caption = "Disaggregated data for KPI 1",
      booktabs=T,
      align=c("l","r","r"), 
      escape = FALSE) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center",
                font_size = 10) %>% 
  pack_rows(index = table(disagg_tab_01$disagg), latex_gap_space = "1em") %>%
...
```{r kpi2.1 chart, echo=FALSE, include=TRUE, fig.cap="\\label{fig:fig2}Cumulative annual results achieved for KPI 2.1 as a result of UK ICF, from April 2011 to March 2023", fig.alt="Cumulative annual results achieved for KPI 2.1 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[2]]
```


```{r, echo=FALSE, fig.align='center', fig.alt="Data are disaggregated by age, clean energy used, disability, geography, and sex"}
disagg_tab_021 <- disagg_table %>% filter(kpi_id=="ICF KPI 02.1") %>% select(-kpi_id)

kable(disagg_tab_021[,-1], 
      caption = "Disaggregated data for KPI 2.1",
      booktabs=T,
      align=c("l","r","r"),
      escape = FALSE) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center",
                font_size = 10) %>% 
  pack_rows(index = table(disagg_tab_021$disagg), latex_gap_space = "1em") %>%
  column_spec(1, bold = T, width = "2in") %>%
  column_spec(2,width = "1.5in") %>%
  column_spec(3,width = "1.5in") %>%
  row_spec(0,bold=TRUE)
...
\begin{genericbox}{Case Study: Enabling Development for Girls Education Programme}

\begin{wrapfigure}{R}{0.3\textwidth}
\centering
\includegraphics{../images/kpi2_case_study.jpg}
\end{wrapfigure}
...
KPI 2.2 is similar to KPI 2.1 but focuses on social institutions\footnote{For this KPI, social institutions are defined as: schools, universities, hospitals, health centres, government institutions, state owned infrastructure and civil society organisations} that have improved access to clean energy, which thereby provides benefits to the wider community. One of the biggest contributors to this KPI was the ‘Somali Health and Nutrition Programme’.

A total of `r tot_022 %>% str_to_lower()` have been supported with improved energy access between 2011 and `r year(Sys.Date())`, through `r pull_data(summary_kpi_progs,"ICF KPI 02.2", "kpi_id", "n")` programmes (Figure \ref{fig:fig3}). There was `r inc_dec_022` of `r diff_022` (`r perc_increase_022`) institutions supported.

 \hfill\break
 
```{r kpi2.2 chart, echo=FALSE, include=TRUE, fig.cap="\\label{fig:fig3}Cumulative annual results achieved for KPI 2.2 as a result of UK ICF, from April 2011 to March 2023", fig.alt="Cumulative annual results achieved for KPI 2.2 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[3]]
```
...
Resilience was improved for a total of `r tot_04 %>% str_to_lower()` across `r summary_kpi_progs %>% filter(if_any(1,~.== "ICF KPI 04")) %>% pull(3)` ICF programmes from April 2011 to March `r year(Sys.Date())` (Figure \ref{fig:fig4}). This was an increase of `r increase_04` (`r perc_increase_04`) from the previous year. The largest increase came from '`r pull(top_progs_04[1,2])`', reporting against this indicator for the first time. Overall, despite increasing, the cumulative results have plateaued this year due to the completion, and closure, of several programmes that have reported large results in the past.
...
```{r kpi4 chart, echo=FALSE, include=TRUE, fig.cap="\\label{fig:fig4}Cumulative annual results achieved for KPI 4 as a result of UK ICF, from April 2011 to March 2023", fig.alt="Cumulative annual results achieved for KPI 4 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[4]]
```


```{r, echo=FALSE, fig.align='center', fig.alt="Data are disaggregated by disability, geography, and sex"}
disagg_tab_04 <- disagg_table %>% filter(kpi_id=="ICF KPI 04") %>% select(-kpi_id)

kable(disagg_tab_04[,-1], 
      caption = "Disaggregated data for KPI 4",
      booktabs=T,
      align=c("l","r","r"),
      escape = FALSE) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center",
                font_size = 10) %>% 
  pack_rows(index = table(disagg_tab_04$disagg), latex_gap_space = "1em") %>%
  column_spec(1, bold = T, width = "2in") %>%
  column_spec(2,width = "1.5in") %>%
  column_spec(3,width = "1.5in") %>%
  row_spec(0,bold=TRUE)
...
```{r kpi6 chart, echo=FALSE, include=TRUE, fig.cap="\\label{fig:fig5}Cumulative annual results achieved for KPI 6 as a result of UK ICF, from April 2011 to March 2023", fig.alt="Cumulative annual results achieved for KPI 6 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[5]]
```



```{r, echo=FALSE, fig.align='center', fig.alt="Data are disaggregated by sector"}
disagg_tab_06 <- disagg_table %>% filter(kpi_id=="ICF KPI 06") %>% select(-kpi_id)

kable(disagg_tab_06[,-1], 
      caption = "Disaggregated data for KPI 6",
      booktabs=T,
      align=c("l","r","r"),
      escape = FALSE) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center",
                font_size = 10) %>% 
  pack_rows(index = table(disagg_tab_06$disagg), latex_gap_space = "1em") %>%
  column_spec(1, bold = T, width = "2in") %>%
  column_spec(2,width = "1.5in") %>%
  column_spec(3,width = "1.5in") %>%
  row_spec(0,bold=TRUE)
...
It is estimated that `r tot_07 %>% str_remove("[ ].*")` megawatts (MW) of clean energy capacity were installed by ICF programmes between 2011 and `r year(Sys.Date())`, `r inc_dec_07` of `r perc_increase_07` on the cumulative installed capacity up to `r year(Sys.Date())-1` (Figure \ref{fig:fig6}). A total of `r pull_data(summary_kpi_progs, filter_col="kpi_id", filter_val="ICF KPI 07", pull_col="n")` programmes have reported against this KPI, 6 programmes more than reported in 2022.  
...
```{r kpi7 chart, echo=FALSE, include=TRUE, fig.cap="\\label{fig:fig6}Cumulative annual results achieved for KPI 7 as a result of UK ICF, from April 2011 to March 2023", fig.alt="Cumulative annual results achieved for KPI 7 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[6]]
```


```{r, echo=FALSE, fig.align='center', fig.alt="Data are disaggregated by geography, whether power is on or off grid, and technology type."}
disagg_tab_07 <- disagg_table %>% filter(kpi_id=="ICF KPI 07") %>% select(-kpi_id)

kable(disagg_tab_07[,-1], 
      caption = "Disaggregated data for KPI 7",
      booktabs=T,
      align=c("l","r","r"),
      escape = FALSE) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center",
                font_size = 10) %>% 
  pack_rows(index = table(disagg_tab_07$disagg), latex_gap_space = "1em") %>%
  column_spec(1, bold = T, width = "2in") %>%
  column_spec(2,width = "1.5in") %>%
  column_spec(3,width = "1.5in") %>%
  row_spec(0,bold=TRUE)
...
\begin{genericbox}{Case Study: Blue Forests Programme}
\begin{wrapfigure}{L}{0.7\textwidth}
\includegraphics{../images/kpi10_case_study.jpg}
\end{wrapfigure}
Starting life as a Darwin Initiative grant project, Blue Forests was scaled up in 2016. Blue Forests implements models for the sustainable management of mangrove habitats across Madagascar and Indonesia. Mangrove habitats are vital for local communities and coastal livelihoods. They provide important sources of income (e.g., carbon credits, honey production, and shrimp farming), provide adaptation and resilience services (e.g., natural defences against floods and coastal erosion) and are integral to the protection of endangered marine biodiversity. 
...
```{r kpi10 chart, echo=FALSE, include=TRUE, fig.cap="\\label{fig:fig8}Cumulative annual results achieved for KPI 10 as a result of UK ICF, from April 2011 to March 2023", fig.alt="Cumulative annual results achieved for KPI 10 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[8]]
```
...
```{r kpi11 chart, echo=FALSE, include=TRUE, fig.cap="\\label{fig:fig9}Cumulative annual results achieved for KPI 11 as a result of UK ICF, from April 2011 to March 2023", fig.alt="Cumulative annual results achieved for KPI 11 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[9]]
```


```{r, echo=FALSE, fig.align='center', fig.alt="Data are disaggregated by climate theme and origin of finance."}
disagg_tab_11 <- disagg_table %>% filter(kpi_id=="ICF KPI 11") %>% select(-kpi_id)

kable(disagg_tab_11[,-1], 
      caption = "Disaggregated data for KPI 11",
      booktabs=T,
      align=c("l","r","r"),
      escape = FALSE) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center",
                font_size = 10) %>% 
  pack_rows(index = table(disagg_tab_11$disagg), latex_gap_space = "1em") %>%
  column_spec(1, bold = T, width = "2in") %>%
  column_spec(2,width = "1.5in") %>%
  column_spec(3,width = "1.5in") %>%
  row_spec(0,bold=TRUE)
...
Mobilised finance measured under this indicator is from non-public sources such as banks (excluding multilateral or regional development banks), private companies, pension funds, non-governmental organisations, Clean Development Mechanism (CDM) financing\footnote{The Clean Development Mechanism (CDM) is a way to finance emissions mitigation projects by selling certified emission reductions, or CERs. For further information, see \href{https://cdm.unfccc.int/}{https://cdm.unfccc.int/}}, voluntary carbon credit market, insurance companies, private savings, family money, entrepreneurs’ own capital and sovereign wealth funds. It includes all types of finance such as equity, debt and guarantees. This indicator helps measure the UK’s contribution to the commitment made by developed countries to mobilise $100 billion of public and private finance per year to help developing countries respond to climate change, as set out in SDG target 13.a.

It is estimated that `r paste0("£", tot_12 %>% str_remove("[ ].*"))`  of private finance has been mobilised up to `r year(Sys.Date())`, `r inc_dec_12` of `r perc_increase_12` when compared with the cumulative results up to `r year(Sys.Date())-1` (Figure \ref{fig:fig10}). Eleven additional programmes reported against this KPI for the first time this year bringing the total to `r summary_kpi_progs %>% filter(if_any(1,~.== "ICF KPI 12")) %>% pull(3)` reporting programmes. Large increases against this KPI were seen in '`r pull(top_progs_kpi_12[1,2])`', '`r pull(top_progs_kpi_12[2,2])`' and '`r pull(top_progs_kpi_12[3,2])`'.
...
```{r kpi12 chart, echo=FALSE, include=TRUE, fig.cap="\\label{fig:fig10}Cumulative annual results achieved for KPI 12 as a result of UK ICF, from April 2011 to March 2023", fig.alt="Cumulative annual results achieved for KPI 12 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[10]]
```


```{r, echo=FALSE, fig.align='center', fig.alt="Data are disaggregated by climate theme, leveraging mechanism, and origin of finance."}
disagg_tab_12 <- disagg_table %>% filter(kpi_id=="ICF KPI 12") %>% select(-kpi_id)

kable(disagg_tab_12[,-1], 
      caption = "Disaggregated data for KPI 12",
      booktabs=T,
      align=c("l","r","r"),
      escape = FALSE) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center",
                font_size = 10) %>% 
  pack_rows(index = table(disagg_tab_12$disagg), latex_gap_space = "1em") %>%
  column_spec(1, bold = T, width = "2in") %>%
  column_spec(2,width = "1.5in") %>%
  column_spec(3,width = "1.5in") %>%
  row_spec(0,bold=TRUE)
...
For KPI 15, programmes score their likelihood of leading to transformational change on a scale from 1 to 5. For this reason, the results cannot be reported cumulatively. Instead, we present the distribution of scores across all reporting programmes (Figure \ref{fig:fig11}).
...
```{r kpi15_scores, echo=FALSE, fig.alt="The scoring system for KPI 15 has 5 categories, ranging from evidence change will not occur, to there is not yet evidence, to there is substantial evidence change will occur."}
...
data.frame(Score, Description) %>% 
  kable( 
      caption = "New scores and definitions for KPI15",
      booktabs=T,
      align=c("c","l")) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center") %>% 
  column_spec(1, bold = T, width = "0.5in") %>%
...
```{r kpi15_plot, echo=FALSE, include=TRUE, fig.width=12,fig.height=10, fig.cap="\\label{fig:fig11}Annual results achieved for KPI 15 from April 2011 to March 2023. This shows the proportion of programmes achieving each score per year on the y axis, and the total number of programmes reporting this KPI is annotated above the bars for context.", fig.alt="Annual results achieved for KPI 15 from April 2011 to March 2023. This shows the proportion of programmes achieving each score per year on the y axis, and the total number of programmes reporting this KPI is annotated above the bars for context.", fig.align='center', fig.width=5.5, fig.height=4}
kpi15_plot_new
```
...
```{r kpi17 chart, echo=FALSE, include=TRUE, fig.cap="\\label{fig:fig12}Cumulative annual results achieved for KPI 17 as a result of UK ICF, from April 2011 to March 2023", fig.alt="Cumulative annual results achieved for KPI 17 as a result of UK ICF, from April 2011 to March 2023", fig.align='center', fig.width=5.5, fig.height=4}
line_plot[[11]]
```


```{r, echo=FALSE, fig.alt="Data are disaggregated by country, ecosystem type, management theme and management practice.", fig.align='center'}
disagg_tab_17 <- disagg_table %>% filter(kpi_id=="ICF KPI 17") %>% select(-kpi_id)

kable(disagg_tab_17[,-1], 
      caption = "Disaggregated data for KPI 17",
      booktabs=T,
      align=c("l","r","r"),
      escape = FALSE) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center",
                font_size = 10) %>% 
  pack_rows(index = table(disagg_tab_17$disagg), latex_gap_space = "1em") %>%
  column_spec(1, bold = T, width = "2in") %>%
  column_spec(2,width = "1.5in") %>%
  column_spec(3,width = "1.5in") %>%
  row_spec(0,bold=TRUE)
...
```{r ta01_plot, echo=FALSE, fig.cap="\\label{fig:fig13}Countries supported with technical assistance reported by ICF TA KPI 1. Darker shades are countries that have been supported.", fig.alt="Countries supported with technical assistance reported by ICF TA KPI 1. The countries are mainly in Africa, Asia and South America", fig.align='center'}
ta01_plot
```


```{r echo=FALSE, warning=FALSE, fig.align='center', fig.alt="One hundred and twenty five countries have been supported with technical assistance"}
...
cbind.fill(chunks[[1]],chunks[[2]],chunks[[3]],chunks[[4]])  %>% as_tibble() %>% mutate_all(replace_na,"-") %>%  
  kable(col.names = NULL,
        caption = "Countries supported by ICF Technical Assistance",
        booktabs=T,
        align=c("l","l","l","l")) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center",
...
```{r, echo=FALSE, fig.align='center', fig.alt="Data are disaggregated by type of technical assistance and sector."}
disagg_tab_ta021<- disagg_table %>% filter(kpi_id=="ICF TA KPI 02.1") %>% select(-kpi_id)

kable(disagg_tab_ta021[,-1], 
      caption = "Disaggregated data for TA KPI 2.1",
      booktabs=T,
      align=c("l","r","r"),
      escape = FALSE) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center",
                font_size = 10) %>% 
  pack_rows(index = table(disagg_tab_ta021$disagg), latex_gap_space = "1em") %>%
  column_spec(1, bold = T, width = "2in") %>%
  column_spec(2,width = "1.5in") %>%
  column_spec(3,width = "1.5in") %>%
  row_spec(0,bold=TRUE)
...
kable(disagg_tab_ta022[,-1], 
      caption = "Disaggregated data for TA KPI 2.2",
      booktabs=T,
      align=c("l","r","r"),
      escape = FALSE) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center",
                font_size = 10) %>% 
  pack_rows(index = table(disagg_tab_ta022$disagg), latex_gap_space = "1em") %>%
  column_spec(1, bold = T, width = "2in") %>%
  column_spec(2,width = "1.5in") %>%
  column_spec(3,width = "1.5in") %>%
  row_spec(0,bold=TRUE)
...
kable(disagg_tab_ta03[,-1], 
      caption = "Disaggregated data for TA KPI 03",
      booktabs=T,
      align=c("l","r","r"),
      escape = FALSE) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center",
                font_size = 10) %>% 
  pack_rows(index = table(disagg_tab_ta03$disagg), latex_gap_space = "1em") %>%
  column_spec(1, bold = T, width = "2in") %>%
  column_spec(2,width = "1.5in") %>%
  column_spec(3,width = "1.5in") %>%
  row_spec(0,bold=TRUE)
...
kable(disagg_tab_ta05[,-1], 
      caption = "Disaggregated data for TA KPI 05",
      booktabs=T,
      align=c("l","r","r"),
      escape = FALSE) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center",
                font_size = 10) %>% 
  pack_rows(index = table(disagg_tab_ta05$disagg), latex_gap_space = "1em") %>%
  column_spec(1, bold = T, width = "2in") %>%
  column_spec(2,width = "1.5in") %>%
  column_spec(3,width = "1.5in") %>%
  row_spec(0,bold=TRUE)
...
\begin{genericbox}{Case Study: Mitigation Action Facility}

\begin{wrapfigure}{R}{0.6\textwidth}
\centering
\includegraphics{../images/ta_case_study.png}
\end{wrapfigure}
The Mitigation Action Facility programme supports mitigation by providing funding to the most transformational parts of Nationally Appropriate Mitigation Actions plans. NAMAs are concrete projects, policies, or programmes that shift a technology or sector in a country onto a low-carbon development trajectory. One supported project - “Mexican New Housing” is based on the whole-house approach focusing on the total energy performance of a building. The overarching goal of the project was to implement mitigation measures which promotes cost-effective energy-efficient building concepts across the housing sector, with a particular focus on low-income housing. The project contributed to the implementation of the mitigation measures in two ways: Firstly, it promoted the penetration of basic efficiency standards in the entire new housing market in Mexico by means of technical assistance to large public housing financiers and housing developers and financial incentives for small and medium sized developers and financial intermediaries. Secondly, it promoted the upgrading of energy efficiency standards to more ambitious levels. 
\end{genericbox}
...
If you have an idea to improve this publication, please get in touch at \href{mailto:statistics@fcdo.gov.uk}{statistics@fcdo.gov.uk}.
...
## Annex 1: ICF expected total programme benefits {#sec:annex1}
...
tpb_table %>% 
  ...
  mutate(`KPI Number`= str_replace_all(`KPI Number`, "I 0", "I ")) %>% 
  kable( 
        caption = "Expected total programme benefits",
        booktabs=T
        ) %>% 
  kable_styling(latex_options = c("hold_position"),
                position = "center",
...
prog_list_table%>% 
  kable( 
        caption = "All programmes that have ever reported ICF results between 2011 and 2023",
        booktabs=T,
        longtable=T
        ) %>% 
...
Cover Image: Raffles Malkoha (\textit{Rhinortha chlorophaea}), Terengganu State Park, Malaysia, where UK PACT facilitates private sector investment into forest protection.  \textit{Syaqil Suhaimi}

Enabling Development for Girls Education Programme case study: \textit{Promoting Equality in African Schools (PEAS)}

Blue Forests case study: \textit{Dr Estelle Jones}

Mitigation Action Facility Case Study: \textit{KFW}


\hfill\break

\small
Jointly published by the Foreign, Commonwealth \& Development Office, the Department for Energy Security and Net Zero, the Department for Science, Innovation and Technology and the Department of Environment, Food \& Rural Affairs.
\hfill\break
...
Website: [www.gov.uk/government/organisations/foreign-commonwealth-developmentoffice](www.gov.uk/government/organisations/foreign-commonwealth-developmentoffice)\newline
Facebook: [www.facebook.com/ForeignCommonwealthAndDevelopmentOffice](www.facebook.com/ForeignCommonwealthAndDevelopmentOffice)\newline
Twitter: @FCDOGovUK\newline
